...
export type HeroScrollVideoProps = {
  ...
  // Next layer — slides up over the fullscreen image after main animation
  nextLayer?: ReactNode;
  nextLayerScrollVh?: number; // scroll height for the exit/next-layer phase (default 150)
...
export const HeroScrollVideo: React.FC<HeroScrollVideoProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const exitMarkerEl = rootRef.current?.querySelector(
        "[data-exit-marker]"
      ) as HTMLElement | null;
      ...
      mainTl = gsap.timeline({
        scrollTrigger: {
          ...
          start: "top top",
          ...(exitMarkerEl
            ? { endTrigger: exitMarkerEl, end: "top top" }
            : { end: "bottom bottom" }),
          scrub: 1.1,
        },
      });
      ...
      // Exit phase — next layer slides up over fullscreen image
      const nLayerEl = nextLayerRef.current;
      if (exitMarkerEl && nLayerEl) {
        const mediaEl = container.querySelector("img, video") as HTMLElement | null;

        gsap.set(nLayerEl, { yPercent: 100 });

        exitTl = gsap.timeline({
          scrollTrigger: {
            trigger: exitMarkerEl,
            start: "top top",
            end: () =>
              `top top-=${(nextLayerScrollVh / 100) * window.innerHeight}px`,
            scrub: 1.1,
          },
        });

        exitTl
          // Next layer slides up from below
          .to(nLayerEl, { yPercent: 0, ease: "expo.out" }, 0)
          // Overlay ("Clarity in Motion") fades as next layer rises
          .to(overlayEl, { opacity: 0, ease: "power2.in" }, 0.15)
          .to(overlayDarkenEl, { opacity: 0 }, 0.15)
          // Image fades out once next layer is mostly covering it
          .to(mediaEl, { opacity: 0, ease: "power2.in" }, 0.4)
          // Next layer background transitions from 50% to fully opaque
          .to(nLayerEl, { backgroundColor: "#0b0c10", ease: "power2.out" }, 0.4);
      }
    })();
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-sticky-scroll
        style={{ height: `${Math.max(150, scrollHeightVh) + (nextLayer ? nextLayerScrollVh : 0)}vh` }}
      >
        {/* Marks where the exit phase begins */}
        {nextLayer ? (
          <div
            data-exit-marker
            style={{
              position: "absolute",
              top: `${Math.max(150, scrollHeightVh)}vh`,
              height: "1px",
              width: "100%",
              pointerEvents: "none",
            }}
        ...
        <div className={`hsv-sticky ${sticky ? "is-sticky" : ""}`}>
          <div className="hsv-media" ref={containerRef}>
            ...
            {/* next layer — slides up over fullscreen image */}
            {nextLayer ? (
              <div className="hsv-next-layer" ref={nextLayerRef}>
                {nextLayer}
              </div>
            ) : null}
          </div>
      ...
      <style>{`
        ...
        .dark .hsv-root, [data-theme="dark"] .hsv-root {
          --bg: #0b0c10;
          --text: #e5e7eb;
          --muted: #9ca3af;
          --muted-bg: rgba(229,231,235,0.08);
